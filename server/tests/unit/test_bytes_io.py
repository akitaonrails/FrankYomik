...
from kindle.image_utils import decode_image_bytes, encode_image_pil
...
class TestLoadPageFromMemory:
    def test_manga_pipeline(self):
        from kindle.processor import load_page_from_memory, PageResult
    ...
    def test_manga_default_name(self):
        from kindle.processor import load_page_from_memory
        img_pil = _make_test_image()
        img_cv = np.array(img_pil)[:, :, ::-1].copy()
